...
void GPRS_Tick(void)
{
	...
	else if (gprs_state == NORMAL_COMM)
	{
		...
		if (cur_time > next_heartbeat_time)
		{
			...
			heartbeat_ErrorCount++;
			if (heartbeat_ErrorCount > 2)
			{
				gprs_state = NEED_INIT;
			}
			else
...
void KeyOpen_Tick(void)
{
	...
	if (0 == KEY_OPEN_Read())
	{
		...
		if (0 == KEY_OPEN_Read())
		{
			...
			if(time_tmp > next_allupload_time)									// 按键开门间隔15s才上报			//
			{
				DBG_SOFT_PutStr("\nKeyopen upload\n");
...
void DefendAlarm_Tick(void)
{
	...
	if (0 == DEFEND_ALARM_Read())
	{
		...
		if (0 == DEFEND_ALARM_Read())
		{
			VoicePlay((uint8)Sound_ShortDI);				// 若拆锁 声音一直响 
			
			if ((next_defend_flag == 0)&&(gprs_state == NORMAL_COMM))
			{
				// 报一次
				//VoicePlay((uint8)Sound_ShortDI);
