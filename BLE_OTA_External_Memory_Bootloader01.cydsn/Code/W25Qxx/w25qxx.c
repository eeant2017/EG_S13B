...
{ 
    ...
    if (id == W25Q80)												// SPI FLASH为W25Q80				//
    {
		...
		{
			FLASH_ENABLE(); 			        					// 使能片选							//
			FLASH_ReadWriteByte(W25X_Enable4ByteAddr);				// 发送进入4字节地址模式指令 		//  
			FLASH_DISABLE();										// 取消片选							//
		}
    }
}
...
{   
	...
	FLASH_ENABLE();
	FLASH_ReadWriteByte(command);									// 发送写取状态寄存器命令    		//
...
{
	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_WriteEnable);   						// 发送写使能  						//
	FLASH_DISABLE();
}
...
{  
	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_WriteDisable);							// 发送写禁止指令    				//
	FLASH_DISABLE();
}
...
uint16_t W25QXX_ReadID(void)
{
	...
	FLASH_ENABLE();				    
	FLASH_ReadWriteByte(W25X_ManufactDeviceID);						// 发送读取ID命令					//
...
{ 
    ...
    {
        FLASH_ReadWriteByte((uint8_t)((ReadAddr) >> 24));
    }
    FLASH_ReadWriteByte((uint8_t)((ReadAddr) >> 16));
...
void W25QXX_Write_Page(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    W25QXX_Write_Enable();
	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_PageProgram);							// 发送写页命令   					//
	// 如果是W25Q256的话地址为4字节的，要发送最高8位													//
    if (W25QXX_TYPE == W25Q256)
    {
        FLASH_ReadWriteByte((uint8_t)((WriteAddr) >> 24)); 
    }
    FLASH_ReadWriteByte((uint8_t)((WriteAddr) >> 16));
...
{                                   
    ...
    W25QXX_Wait_Idle();
  	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_ChipErase);							// 发送片擦除命令					//
	FLASH_DISABLE();
	W25QXX_Wait_Idle();
}
...
{  
    ...
    W25QXX_Wait_Idle();
  	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_SectorErase);
    if (W25QXX_TYPE == W25Q256)
    {
        FLASH_ReadWriteByte((uint8_t)((Dst_Addr) >> 24)); 
    }
...
{  
  	FLASH_ENABLE();
    FLASH_ReadWriteByte(W25X_ReleasePowerDown);
	FLASH_DISABLE();
    CyDelayUs(3);
}
