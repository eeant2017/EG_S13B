...
{
    0x0012u,    /* Handle of the BTS service */
    {
        
        /* Command characteristic */
        {
            0x0014u, /* Handle of the Packet characteristic */ 
...
* 
*  \param CYBLE_GATTS_WRITE_REQ_PARAM_T * eventParam: The pointer to data that 
*                         came with a Write Request for BTS.
* 
* \return
...
*            received with a Prepare Write Request event for the Bootloader
*            service.
* 
******************************************************************************/
...
*           with a Write Request for the Bootloader service.
* 
* \return
*  None.
* 
...
*  CyBle_GattsNotification() to sent the data. If a notification is accepted,
*  the function returns CYRET_SUCCESS. The timeOut parameter is ignored in
*  this case.
* 
...
* 
*  \param uint8 *data:   The pointer to the buffer to store data from the host 
*                        controller.
*  \param uint16 size:   The number of bytes to read into the data buffer.
*  \param uint16 *count: The pointer to where the BLE component will write the 
*                        count of the number of read bytes.
*  \param uint8 timeOut: The amount of time (in units of 10 milliseconds) for 
*                        which the BLE component should wait before indicating 
...
cystatus CyBLE_CyBtldrCommRead(uint8 data[], uint16 size, uint16 *count, uint8 timeOut)
{
    ...
    /* Convert from 10mS check to 1mS checks */
    timeoutMs = ((uint32) 10u * timeOut);
